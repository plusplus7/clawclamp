...
Clawclamp 自带一个偏运维风格的紧凑 Web UI，核心围绕三件事展开：策略编写、审计回看，以及运行时模式和临时授权操作。

- `策略面板`：在浏览器里增删改查 Cedar policy
- `审计日志`：查看允许、拒绝、灰度放行的调用记录和请求细节
- `模式与授权`：切换 `gray` / `enforce`，并发放短期授权

## 截图

### 策略面板

![策略面板](./screenshots/policy-lab.png)
...
![模式与授权](./screenshots/policy-mode.png)
